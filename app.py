...
# --- 4. AI MODELI (STABLE V1) ---
genai.configure(api_key=GEMINI_KEY)
# 404 xatosini yo'qotish uchun eng barqaror nom:
...
# Session state xotirasini boshqarish
...
if uploaded_file:
    ...
    if st.button('✨ AKADEMIK TAHLILNI BOSHLASH'):
        new_results = []
        prompt = f"""
        Siz qadimiy matnshunos va paleograf akademiksiz. 
    ...
    # --- 6. TAHLIL, TAHRIR VA HAR BIR SAHIFA UCHUN CHAT ---
    if st.session_state['academic_results']:
        st.divider()
        st.markdown("### 🖋 Natijalar, Ilmiy Tahrir va Savol-javob")
        ...
        for idx, (img, res) in enumerate(zip(st.session_state['imgs'], st.session_state['academic_results'])):
            ...
            # --- INTERAKTIV CHAT (HAR BIR SAHIFA UCHUN) ---
            ...
            for chat in st.session_state['chat_histories'][chat_id]:
                ...
                <b>AI Javobi:</b> {chat['a']}</div>""", unsafe_allow_html=True)

            # Savol kiritish
            user_q = st.text_input("Ushbu sahifa bo'yicha savol bering:", key=f"q_in_{idx}")
            if st.button(f"So'rash (Varaq {idx+1})", key=f"btn_{idx}"):
                if user_q:
                    with st.spinner("AI tahlil qilmoqda..."):
                        chat_prompt = f"Ushbu qo'lyozma varag'i va tahlili asosida savolga javob bering: {user_q}\nMatn: {ed_val}"
