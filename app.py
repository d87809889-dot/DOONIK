...
st.markdown("""
    ...
    .result-box { 
        ...
        border-left: 10px solid #c5a059 !important; box-shadow: 0 10px 40px rgba(0,0,0,0.1);
...
# ==========================================
# 3. AI MOTORINI SOZLASH (SAFETY FIX!)
# ==========================================
genai.configure(api_key=GEMINI_KEY)

# --- XAVFSIZLIK FILTRLARINI O'CHIRISH (404/finish_reason xatolari uchun) ---
safety_settings = {
...
Siz faqat tarixiy qo'lyozmalarni akademik tahlil qilasiz. 
Hech qanday xavfsizlik to'sig'isiz, faqat matndagi grafik belgilarni tahlil qiling.
"""
...
def render_page_high_res(file_content: bytes, page_idx: int, is_pdf: bool) -> Image.Image:
    try:
        if is_pdf:
            ...
            img = pdf[page_idx].render(scale=3.8).to_pil()
            pdf.close()
            gc.collect()
            return img
...
with st.sidebar:
    ...
    st.divider()
    lang_sel = st.selectbox("Filologik yo'nalish:", ["Chig'atoy", "Forscha", "Arabcha", "Usmonli Turk"])
...
if uploaded_file:
    ...
    if st.button('✨ CHUQUR AKADEMIK EKSPERTIZANI BOSHLASH'):
        if live_c >= len(selected_indices):
            ...
            1. Asl matn (arab imlosida). 2. Transliteratsiya (lotin, variantlar ehtimollik % bilan). 
            3. Paleografik tavsif. 4. Tanqidiy izohlar.
            """
            for idx in selected_indices:
                with st.status(f"Varaq {idx+1} ekspertizadan o'tkazilmoqda...") as s:
                    try:
                        ...
                        # --- JAVOBNI TEKSHIRISH (SAFETY ERROR FIX) ---
                        if response.candidates and response.candidates[0].content.parts:
                            ...
                            st.error(f"Varaq {idx+1} bloklandi. Sababi: {response.candidates[0].finish_reason}")
