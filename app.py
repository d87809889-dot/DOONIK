...
st.markdown("""
    ...
    .result-box {
        ...
        color: #1a1a1a !important; font-size: 17px !important;
    }
...
# --- 4. AI MODELINI TO'G'RI SOZLASH (SIZDA ISHLAGAN NOM BILAN) ---
genai.configure(api_key=GEMINI_KEY)
# SIZNING LOGSDA ISHLAYOTGANINI TASDIQLAGAN NOM:
    ...
    if st.session_state['academic_results']:
        ...
        for idx, (img, res) in enumerate(zip(st.session_state['imgs'], st.session_state['academic_results'])):
            ...
            if st.button(f"So'rash {idx+1}", key=f"btn_{idx}"):
                if user_q:
                    chat_prompt = f"Qo'lyozma matni asosida savolga akademik javob ber: {user_q}\nMatn: {ed_val}"
